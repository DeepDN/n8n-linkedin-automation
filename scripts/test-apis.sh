...
if [ -n "$LINKEDIN_CLIENT_ID" ] && [ -n "$LINKEDIN_CLIENT_SECRET" ]; then
    ...
    echo -e "${YELLOW}⚠️  LinkedIn credentials not provided - skipping test${NC}"
    total_tests=$((total_tests - 1))
fi
...
EOF
    ...
    echo -e "${YELLOW}⚠️  Google Sheets credentials not provided - skipping test${NC}"
    total_tests=$((total_tests - 1))
fi
...
if curl -s --connect-timeout 5 "$n8n_url" > /dev/null 2>&1; then
    ...
    echo -e "${YELLOW}⚠️  n8n Local: Instance not running (this is OK if using n8n Cloud)${NC}"
    total_tests=$((total_tests - 1))
fi
...
if [ $passed_tests -eq $total_tests ]; then
    ...
    echo -e "\n${YELLOW}⚠️  Most tests passed. Review failed tests and fix issues before proceeding.${NC}"
    exit 1
else
...
if ! command -v n8n &> /dev/null; then
    echo -e "${YELLOW}⚠️  n8n not installed globally. Run: npm install -g n8n${NC}"
fi

# Check for firewall issues
if command -v ufw &> /dev/null && ufw status | grep -q "Status: active"; then
    echo -e "${YELLOW}⚠️  UFW firewall is active. Ensure ports 5678 is allowed if testing locally.${NC}"
fi
